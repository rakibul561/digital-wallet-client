...
import { TrendingUp, Users, DollarSign, Activity, Wallet } from "lucide-react"
...
const Analytics = () => {
  ...
  const totalTransactionValue = transactions.reduce((sum: number, txn: any) => sum + (txn.amount ?? 0), 0)

  // Calculate average transaction
  const avgTransaction = allTransactionCount > 0 ? totalTransactionValue / allTransactionCount : 0
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30 dark:from-gray-950 dark:via-gray-900 dark:to-blue-950/30">
      <div className="p-8 space-y-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <Card className="border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">Monthly Transaction Volume</h3>
                  <p className="text-gray-600 dark:text-gray-400 text-sm">Revenue and transaction trends</p>
                </div>
          ...
          <Card className="border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-xl">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white">Top Wallet Distribution</h3>
                  <p className="text-gray-600 dark:text-gray-400 text-sm">User transaction volume breakdown</p>
                </div>
        ...
        <Card className="border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-xl">
          <CardContent className="p-6">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">Transaction Insights</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center p-4 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/50 dark:to-indigo-950/50">
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                  {transactions.length > 0 ? Math.max(...transactions.map((t: any) => t.amount ?? 0)).toLocaleString() : 0}
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Highest Transaction</div>
              </div>
              <div className="text-center p-4 rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/50 dark:to-emerald-950/50">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {monthlyTransactions.length > 0 ? Math.max(...monthlyTransactions.map(m => m.count)) : 0}
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Peak Monthly Transactions</div>
              </div>
              <div className="text-center p-4 rounded-xl bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/50 dark:to-pink-950/50">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">Active Wallets</div>
              </div>
            </div>
          </CardContent>
        </Card>
