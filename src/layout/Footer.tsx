import Logo from "@/assets/icons/Logo";
import { Link } from "react-router";

const Footer = () => {
  return (
    <footer className="bg-black border-t-4 border-primary text-white">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        {/* Top Section */}
        <div className="grid grid-cols-1 gap-10 md:grid-cols-3">
          {/* Logo & Socials */}
          <div>
            <div className="flex items-center gap-3">
              <Link to="/" className="text-primary hover:opacity-90">
                <Logo />
              </Link>
              <h2 className="text-2xl font-bold text-[#FF4D00]">Easy Pay</h2>
            </div>

            <p className="mt-4 max-w-sm text-gray-400 text-sm sm:text-base">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Esse non
              cupiditate quae nam molestias. Your trusted payment partner.
            </p>

            {/* Social Links */}
            <ul className="mt-6 flex flex-wrap items-center gap-5 sm:gap-6">
              {/* Facebook */}
              <li>
                <a
                  href="#"
                  target="_blank"
                  rel="noreferrer"
                  className="hover:text-primary transition"
                >
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
                    <path
                      fillRule="evenodd"
                      d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 
                      3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797
                      c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 
                      2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 
                      1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 
                      21.128 22 16.991 22 12z"
                      clipRule="evenodd"
                    />
                  </svg>
                </a>
              </li>

              {/* Instagram */}
              <li>
                <a
                  href="#"
                  target="_blank"
                  rel="noreferrer"
                  className="hover:text-primary transition"
                >
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
                    <path
                      fillRule="evenodd"
                      d="M12.315 2c2.43 0 2.784.013 3.808.06 
                      1.064.049 1.791.218 2.427.465a4.902 
                      4.902 0 011.772 1.153 4.902 4.902 0 
                      011.153 1.772c.247.636.416 1.363.465 
                      2.427.048 1.067.06 1.407.06 
                      4.123v.08c0 2.643-.012 2.987-.06 
                      4.043-.049 1.064-.218 1.791-.465 
                      2.427a4.902 4.902 0 01-1.153 
                      1.772 4.902 4.902 0 01-1.772 
                      1.153c-.636.247-1.363.416-2.427.465
                      -1.067.048-1.407.06-4.123.06h-.08c-2.643 
                      0-2.987-.012-4.043-.06-1.064-.049-1.791
                      -.218-2.427-.465a4.902 4.902 0 
                      01-1.772-1.153 4.902 4.902 0 
                      01-1.153-1.772c-.247-.636-.416
                      -1.363-.465-2.427-.047-1.024
                      -.06-1.379-.06-3.808v-.63c0
                      -2.43.013-2.784.06-3.808.049
                      -1.064.218-1.791.465-2.427a4.902
                      4.902 0 011.153-1.772A4.902 4.902
                      0 015.45 2.525c.636-.247 1.363
                      -.416 2.427-.465C8.901 2.013 9.256
                      2 11.685 2h.63z"
                      clipRule="evenodd"
                    />
                  </svg>
                </a>
              </li>

              {/* Twitter */}
              <li>
                <a
                  href="#"
                  target="_blank"
                  rel="noreferrer"
                  className="hover:text-primary transition"
                >
                  <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                  >
                    <path d="M8.29 20.251c7.547 0 
                    11.675-6.253 11.675-11.675 0-.178 
                    0-.355-.012-.53A8.348 8.348 0 
                    0022 5.92a8.19 8.19 0 
                    01-2.357.646 4.118 4.118 0 
                    001.804-2.27 8.224 8.224 0 
                    01-2.605.996 4.107 4.107 0 
                    00-6.993 3.743 11.65 11.65 0 
                    01-8.457-4.287 4.106 4.106 0 
                    001.27 5.477A4.072 4.072 0 
                    012.8 9.713v.052a4.105 4.105 0 
                    003.292 4.022 4.095 4.095 0 
                    01-1.853.07 4.108 4.108 0 
                    003.834 2.85A8.233 8.233 0 
                    012 18.407a11.616 11.616 0 
                    006.29 1.84" />
                  </svg>
                </a>
              </li>
            </ul>
          </div>

          {/* Links Section */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 sm:gap-10 md:col-span-2">
            {/* Services */}
            <div>
              <p className="font-semibold text-white">Services</p>
              <ul className="mt-4 space-y-2 text-sm">
                {["1on1 Coaching", "Company Review", "Accounts Review", "HR Consulting", "SEO Optimisation"].map((item) => (
                  <li key={item}>
                    <a href="#" className="hover:text-primary transition">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Company */}
            <div>
              <p className="font-semibold text-white">Company</p>
              <ul className="mt-4 space-y-2 text-sm">
                {["About", "Meet the Team", "Accounts Review"].map((item) => (
                  <li key={item}>
                    <a href="#" className="hover:text-primary transition">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Helpful Links */}
            <div>
              <p className="font-semibold text-white">Helpful Links</p>
              <ul className="mt-4 space-y-2 text-sm">
                {["Contact", "FAQs", "Live Chat"].map((item) => (
                  <li key={item}>
                    <a href="#" className="hover:text-primary transition">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>

            {/* Legal */}
            <div>
              <p className="font-semibold text-white">Legal</p>
              <ul className="mt-4 space-y-2 text-sm">
                {["Accessibility", "Returns Policy", "Refund Policy", "Hiring-3 Statistics"].map((item) => (
                  <li key={item}>
                    <a href="#" className="hover:text-primary transition">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>

        {/* Bottom Section */}
        <div className="mt-10 border-t border-gray-700 pt-6 text-center text-sm text-gray-400">
          &copy; {new Date().getFullYear()} Easy Pay. All rights reserved.
        </div>
      </div>
    </footer>
  );
};

export default Footer;
